namespace AldaJoyeros.Services
{
    ...
    {
        // Mapeo de los primeros 2 dígitos del CP a provincias de España
        private static readonly Dictionary<string, string> _prefijosProvincias = new()
        {
            { "01", "Álava" },
            { "02", "Albacete" },
            { "03", "Alicante" },
        ...
        // Ciudades principales por provincia (las más comunes)
        private static readonly Dictionary<string, List<string>> _ciudadesPorProvincia = new()
        {
        ...
        // Mapeo específico de algunos códigos postales comunes a ciudades
        private static readonly Dictionary<string, string> _cpCiudadesEspecificas = new()
        {
            // Madrid capital
            ...
            // Algunas otras ciudades importantes
        ...
        public List<string> ObtenerCiudadesPorProvincia(string provincia)
        {
            ...
            if (_ciudadesPorProvincia.TryGetValue(provincia, out var ciudades))
            {
        ...
        public CodigoPostalInfo? BuscarPorCodigoPostal(string codigoPostal)
        {
            ...
            var prefijo = codigoPostal.Substring(0, 2);

            if (!_prefijosProvincias.TryGetValue(prefijo, out var provincia))
                return null;

            // Buscar ciudad específica o usar la capital de provincia
            string ciudad;
            if (_cpCiudadesEspecificas.TryGetValue(codigoPostal, out var ciudadEspecifica))
            {
                ciudad = ciudadEspecifica;
            }
            else
            {
                // Si no hay ciudad específica, usar la primera de la lista (capital)
                ciudad = _ciudadesPorProvincia.TryGetValue(provincia, out var ciudades) && ciudades.Count > 0
                    ? ciudades[0]
                    : provincia;
            }
