...
namespace AldaJoyeros.Services.Implementations
{
    public class PedidoService : IPedidoService
    {
        ...
        public async Task<IEnumerable<PedidoDto>> GetByUsuarioIdAsync(long usuarioId)
        {
        ...
        public async Task<PedidoDto> UpdateEstadoAsync(long id, PedidoUpdateEstadoDto pedidoUpdateEstadoDto)
        {
            var pedido = await _pedidoRepository.GetByIdAsync(id);
            if (pedido == null)
            {
        ...
        {
            if (!await _pedidoRepository.ExistsAsync(id))
            {
                throw new KeyNotFoundException("Pedido no encontrado");
            }
